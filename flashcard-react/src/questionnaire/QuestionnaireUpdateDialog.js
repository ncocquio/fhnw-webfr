...
export default class QuestionnaireUpdateDialog extends React.Component {
  ...
  render() {
    return (
      <div>
        <Button color="primary" onClick={this.open} className="float-right">
          Edit
        </Button>
  ...
  submit = () => {
    this.props.updateHandler({id:this.props.questionnaire.id,
      title:this.state.title, description:this.state.description});
    this.setState({ showModal: false, title: this.props.questionnaire.title,
